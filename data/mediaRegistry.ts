...
import { getR2ThumbKey } from "@/data/r2ThumbIndex";
 ...
 * Most posts use:
 *   full/preview → `{postId}.mp4` from data/r2VideoIndex.ts
 *   thumb → `thumbnail/{postId}.jpg` from data/r2ThumbIndex.ts
 * Re-run `npm run sync-r2` after uploads.
 */
...
function defaultR2Thumb(postId: string): MediaSource | undefined {
  const key = getR2ThumbKey(postId);
  if (!key) return undefined;
  return { type: "r2", key };
}
...
export function getMediaSource(postId: string, kind: MediaKind): MediaSource | undefined {
  ...
  if (kind === "thumb") {
    return defaultR2Thumb(id) ?? placeholderThumb(id);
  }
...
export function isR2MediaSource(
  source: MediaSource | undefined
): source is { type: "r2"; key: string } {
